...
class Generator(nn.Module):
    def __init__(self, noise_dim, condition_dim=40, output_dim=108, embedding_dim=100):
...
class Discriminator(nn.Module):
    def __init__(self, input_dim=108, condition_dim=40, embedding_dim=100):
...
#data
data = np.load("train_cGAN_psl_40.npy", allow_pickle=True)
...
for epoch in range(num_epochs):
    ...
    if (epoch) % save_epoch == 0:
        torch.save(generator.state_dict(), "models/G_ts_psl_40_linear-%d.model" % (epoch))
        torch.save(discriminator.state_dict(), "models/D_ts_psl_40_linear-%d.model" % (epoch))